...
<figure align="center">
  <img src="../assets/ch02/ch02_data_type_lab_receipt.png" alt="数据类型实验回执" width="82%" style="max-width:900px; display:block; margin:0 auto;" />
  <figcaption><strong>图2-20 数据类型实验回执</strong>：同一份回执里，`str` 保存文字标签，`list` 保存分数序列，`dict` 保存完整记录，`float` 保存反应时，`bool` 保存判断结果，`None` 表示暂时缺失。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_stroop_dataset_pack.png" alt="Stroop 数据类型包" width="82%" style="max-width:900px; display:block; margin:0 auto;" />
  <figcaption><strong>图2-21 Stroop 数据类型包</strong>：`list` 保存多条 trial，`dict` 保存一条 trial，`str` 保存刺激标签，`float` 保存反应时，`bool` 保存正确与一致判断，`None` 保存暂无备注。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_data_type_specimen_cabinet.png" alt="数据类型标本柜" width="82%" style="max-width:900px; display:block; margin:0 auto;" />
  <figcaption><strong>图2-22 数据类型标本柜</strong>：`11_make_data_type_specimen_cabinet.py` 把 Stroop 记录拆成一格一格的类型标本，让你看到每种类型在同一个任务里的角色。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_type_decision_cards_preview.png" alt="类型选择卡片预览" width="82%" style="max-width:900px; display:block; margin:0 auto;" />
  <figcaption><strong>图2-23 类型选择卡片预览</strong>：配套脚本会生成 Markdown 卡片和预览图，把“选类型”变成可复习的小工具。</figcaption>
</figure>
...
如果报告显示 `14/14 ready`，说明本章关键产物已经生成。这个检查不要求你一次写对所有脚本；它的价值是把“哪里缺了”明确指出来，让你能回到对应脚本补跑。

<figure align="center">
  <img src="../assets/ch02/ch02_data_type_runtime_evidence.png" alt="PowerShell风格的数据类型运行证据总览" width="82%" style="max-width:900px; display:block; margin:0 auto;" />
  <figcaption><strong>图2-24 PowerShell风格的数据类型运行证据</strong>：`12_make_data_type_runtime_evidence.py` 检查本章学习报告、类型选择工具、Stroop 数据包和类型标本柜是否已经生成，把“我理解类型”变成可复盘的项目证据。</figcaption>
</figure>
...
请检查报告中是否显示 `14/14 ready`。如果出现 `MISSING`，先不要急着改代码，回到缺失路径对应的脚本，把它补跑一遍。这个练习的目的，是让你在学完正文后再做一次自检：哪些产物已经准备好，哪些路径还需要补跑脚本。
...
│   └── ch02_user_experience_review.md
├── scripts/
│   ├── check_links.py
