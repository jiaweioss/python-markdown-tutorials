...
本章的读法很简单：先读文字说明和命令，再看真实运行截图，最后核对四个记录：**项目目录、解释器路径、pip 路径、运行输出**。不同电脑的软件版本可能不同，按钮位置也可能变化，但这四个记录不会变。

很多初学者不是卡在复杂算法上，而是卡在第一天：路径错了、解释器选错了、包装进另一个 Python 里了。电脑并没有在嘲讽你，它只是很认真地说：“这个地址我找不到。”本章要做的，就是把这种混乱拆成几个可以核对的小记录。
...
| 第三部分：从脚本到记录 | 1.8-1.18 | 写第一段代码，并把运行结果留下来 | `reports/ch01_environment_log.txt` |
...
先不要急着打开漂亮的 IDE。很多环境问题并不是“Python 没装好”，而是你站错了目录、选错了解释器，或者 pip 把包送到了另一个 Python 里。PowerShell 的作用很朴素：先证明这台电脑现在能找到 Python、能找到 pip、能运行本章脚本。
...
<figure align="center">
  ...
  <figcaption><strong>图1-3 本机 PowerShell 环境检查</strong>：真实运行时至少要看到 Python 版本、pip 路径、解释器路径和当前项目目录；截图里这些记录已经一次跑通。</figcaption>
</figure>
...
如果你能看到脚本输出，就先小小松一口气。此时你不是“好像装好了 Python”，而是已经有了可复现记录：路径对、解释器对、脚本能运行。学习编程最珍贵的不是一开始就写出华丽代码，而是知道怎样证明一件事真的发生了。接着可以连续运行版本检查、pip 检查、Hello 脚本和环境检查脚本，把记录一次收齐。

<figure align="center">
  <img src="../assets/ch01/ch01_powershell_run_scripts.png" alt="PowerShell 连续运行本章脚本截图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图1-7 PowerShell 连续运行脚本</strong>：能在终端里连续跑通版本检查、pip 检查、Hello 脚本和环境检查脚本，才算本章环境真正通电。</figcaption>
</figure>

最后，把本章最关键的运行记录放到一张总览图里。它不是装饰图，而是环境配置的“通电证明”：以后哪里出问题，就回头核对这张图里出现过的路径、解释器和输出。
...
<figure align="center">
  ...
  <figcaption><strong>图1-8 PowerShell 本章运行记录总览</strong>：从版本、pip、Hello 脚本、环境检查脚本到通电日志，全部在同一个项目目录里跑通；这张图就是本章最重要的“我真的配置好了”记录。</figcaption>
</figure>
...
最后把解释器路径和 Run 输出放在一起核对。上面看“谁在运行”，下面看“运行出了什么”；这两个记录对上，IDE 配置才不是凭感觉成功。
...
<figure align="center">
  <img src="../assets/ch01/ch01_runtime_environment_side_by_side.png" alt="PowerShell 与 PyCharm 运行记录同屏截图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图1-21 PowerShell 与 PyCharm 同屏核对</strong>：左边是本章 PowerShell 运行记录，右边是真实 PyCharm Run 窗口位置；在你的电脑上，右侧也应该运行本章脚本，并对齐同一个解释器。</figcaption>
</figure>
...
第0章已经讲过课程大地图，这里只保留和环境配置有关的三个概念：
...
#### 1.6.1 互联网梗图：为什么 `import antigravity` 会成为经典？

<figure align="center">
  <img src="../assets/ch01/ch01_xkcd_antigravity_card.png" alt="xkcd 353 Python 梗图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图1-23 xkcd 353：Python</strong>：梗图负责降低陌生感，正式学习仍要回到 `import`、模块和运行验证。</figcaption>
</figure>

这张 xkcd 梗图不是说 Python 真能让人飞起来，而是在夸张表达一种体验：短代码、快反馈、立刻看见结果。它也顺手带出一个正式概念：`import` 是导入模块。以后你导入的每个工具，最终都要落到某个具体 Python 环境里，所以本章才一直核对解释器和 pip。
...
<figure align="center">
  ...
  <figcaption><strong>图1-30 第一段脚本反馈回路</strong>：一个 `.py` 文件不是孤零零地躺在文件夹里；它会被解释器运行，在终端里给出反馈，再把结果写成日志或报告。第一章真正要建立的，就是这条从脚本到记录的闭环。</figcaption>
</figure>
...
比起直接写 `pip install pandas`，`python -m pip` 更像在快递单上写清楚收件人：“请把包送到当前这个 Python 解释器那里。”如果将来出现 `ModuleNotFoundError`，先别急着重装，先回到截图里的学习记录链：解释器是谁，pip 属于谁，脚本从哪个目录运行。
...
<figure align="center">
  ...
  <figcaption><strong>图1-33 pathlib 路径检查</strong>：`Path.cwd()`、`/` 拼接、`.name`、`.parent` 和 `.suffix` 能把路径拆成可检查的记录，少靠猜，多靠输出。</figcaption>
</figure>
...
本章收尾不再重复第0章的项目大设定，只保留最短上机路线。你可以把它当成“环境配置通关单”：
...
<figure align="center">
  <img src="../assets/ch01/ch01_powershell_submission_evidence.png" alt="PowerShell 第1章学习成果记录截图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图1-34 第1章学习成果记录</strong>：Hello 脚本、环境检查、通电日志和极简实验预告都能在同一目录下运行，才说明第一章的环境链路真正接通。</figcaption>
</figure>

学习成果记录不需要花哨。它像实验记录里的“原始结果”：命令跑过，输出出现，日志文件存在，下一次别人也能沿着同样路线复现。环境配置的自信，不是靠感觉来的，是靠记录一点点积累出来的。

动手练习保留 4 个就够：

1. 运行 `01_hello_python.py`，截图或复制输出。
2. 运行 `02_environment_check.py`，写下 Python 版本、解释器路径和当前目录。
3. 故意写一次 `print(student_name)`，观察 `NameError`，再用 `student_name = "Alice"` 修复。
4. 修改 `04_show_me_the_python.py`，把你的姓名和本学期最想做的小作品写进日志。

最常见的 3 个坑也只记 3 句：

1. `pip install` 成功，不等于装进了正在运行的 Python。
2. 程序找不到文件，先看当前工作目录。
3. 报错不是判决书，是线索单。
...
会读报错，才算真的开始会用 Python。以后环境出问题时，优先拿出三样记录：解释器路径、pip 路径、当前工作目录。
