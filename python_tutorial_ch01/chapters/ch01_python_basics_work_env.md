...
欢迎来到第 1 章。这一章不展开课程地图，只完成一件事：**让 Python 在你的电脑上、在 PowerShell 里、在 PyCharm 里都能跑同一批脚本**。

本章的读法很简单：先读文字说明和命令，再看真实运行截图，最后核对四个证据：**项目目录、解释器路径、pip 路径、运行输出**。不同电脑的软件版本可能不同，按钮位置也可能变化，但这四个证据不会变。

很多初学者不是卡在复杂算法上，而是卡在第一天：路径错了、解释器选错了、包装进另一个 Python 里了。电脑并没有在嘲讽你，它只是很认真地说：“这个地址我找不到。”本章要做的，就是把这种混乱拆成几个可以核对的小证据。跟着做，把这四个证据对上，这一章就算过。
...
在继续敲命令之前，先把几个名字讲白。很多同学高中接触 Python 时，会说“我下了一个编译器，然后在里面写代码”，但实际上，Python 不是像 C 语言那样先编译成一个 `.exe` 再运行。你安装的 Python，核心是一个**解释器**：它负责读懂 `.py` 文件，并把里面的语句执行出来。
...
这里先不突然引入 `sum()` 和 `range()`。第一遍只看懂两件事：第一行把计算结果交给 `total`，第二行用 `print()` 把结果显示出来。`sum()`、`range()` 会在第 2 章讲循环时再正式出现。
...
#### 1.9.1 先学会读 `名字(...)`：这是一次函数调用

从下面开始，代码里会不断出现带圆括号的名字，例如：

```python
print("Hello, Python!")
```

这不是需要整块背下来的咒语，而是在**调用函数**。函数可以先理解成 Python 已经准备好的一件小工具：你把材料交给它，它完成一个动作，有时还会把结果交还给你。

把这一行拆开看：

| 部分 | 含义 |
| --- | --- |
| `print` | 函数名，表示要使用“输出到屏幕”这个工具 |
| `(` 和 `)` | 表示现在要调用它 |
| `"Hello, Python!"` | 传给函数的参数，也就是这次要处理的材料 |

参数写在括号里。没有参数时，括号仍然要保留，例如 `print()` 会输出一个空行。多个参数用英文逗号隔开：

```python
print("姓名：", "小明")
```

有些函数会**返回一个结果**。返回值可以交给变量保存：
...
`input()` 先显示提示文字，再等待键盘输入；按下回车后，它把输入内容作为字符串返回。`name = ...` 的意思是把这个返回值保存到变量 `name`。相反，`print()` 的主要任务是产生输出，初学阶段不需要保存它的返回值。
...
- `message.upper()` 是**方法调用**：先有一个字符串对象 `message`，再请它执行自己的 `upper()` 方法。
- `time.sleep(0.5)` 是**模块中的函数**：先导入 `time` 工具箱，再使用里面的 `sleep()`。

读陌生调用时，先问四个问题：它叫什么、括号里要放什么、会返回什么、会不会修改原来的对象。后面的教程第一次使用新函数或新方法时，也会按这四点说明。
...
这段项目代码会提前借用第 3 章的文件工具，但不会要求你现在默写。先看懂调用地图，再完整运行：

| 写法 | 现在只需要知道什么 |
| --- | --- |
| `Path.cwd()` | 返回当前工作目录 |
| `Path("reports")` | 创建一个表示路径的对象，此时还没有创建文件夹 |
| `report_dir.mkdir(exist_ok=True)` | 真正创建文件夹；已经存在时不报错 |
| `datetime.datetime.now()` | 返回当前日期和时间 |
| `.strftime("...")` | 按指定格式把时间转换成文字 |
| `log_file.write_text(...)` | 把文字写入文件；`encoding="utf-8"` 指定中文编码 |

注意 `sys.executable` 后面没有圆括号，所以它不是函数调用，而是模块里保存的一项信息：当前 Python 解释器的路径。能区分“带括号的动作”和“不带括号的数据”，以后读代码会轻松很多。
...
| 调用 | 输入 | 返回值或作用 |
| --- | --- | --- |
| `time.sleep(0.5)` | 秒数 `0.5` | 暂停半秒，不需要保存结果 |
| `time.perf_counter()` | 无 | 返回一个适合计算时间间隔的高精度计时值 |
| `input("提示")` | 提示文字 | 等待输入，并返回字符串 |
| `round(number, 2)` | 数字和保留位数 | 返回保留 2 位小数的新数字 |

程序先保存开始时刻，输入完成后再取一次时刻，两者相减就是经过的时间。`response == correct_key` 中的 `==` 是“是否相等”，它会得到 `True` 或 `False`；条件判断会在第 2 章正式学习。
