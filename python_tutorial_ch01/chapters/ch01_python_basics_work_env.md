...
先从 Python 的发明者 Guido van Rossum 看起：这门语言最迷人的地方，不是把语法弄得神秘，而是尽量让程序员少绕路。
...
如果你看到的是执行策略提示，不要急着重装 Python。先用下面这张图里的备用运行方式核对：脚本到底是不是由 `.venv` 里的 Python 执行。
...
如果你能看到脚本输出，就先小小松一口气。此时你不是“好像装好了 Python”，而是已经有了可复现证据：路径对、解释器对、脚本能运行。学习编程最珍贵的不是一开始就写出华丽代码，而是知道怎样证明一件事真的发生了。接着可以连续运行版本检查、pip 检查、Hello 脚本和环境检查脚本，把证据一次收齐。
...
最后，把本章最关键的运行证据放到一张总览图里。它不是装饰图，而是环境配置的“通电证明”：以后哪里出问题，就回头核对这张图里出现过的路径、解释器和输出。
...
Python 社区很会开玩笑，但好笑背后通常藏着一个认真概念。`import antigravity` 这个梗，正好可以用来认识“导入模块”。
...
如果把 Python 放进心理学和科研场景，它最先解决的不是“写复杂算法”，而是让刺激、反应、时间、数据和报告能被稳定记录。
...
环境配置里最容易让人怀疑人生的角色，往往不是 Python，而是 pip。它像快递员：包确实送到了，但必须确认送到的是正在运行的那个解释器。
...
当电脑里有多个 Python、多个 pip、多个项目目录时，环境就会像一张绕在一起的线团。先看一张梗图，再把它拆成可检查的三条证据。
...
报错不是电脑在发脾气，它更像路牌：告诉你哪一段路没走通。先把常见错误类型看成一张地图，再逐个拆开。
...
<figure align="center">
  <img src="../assets/ch01/ch01_powershell_submission_evidence.png" alt="PowerShell 第1章提交证据截图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图1-34 第1章提交证据</strong>：Hello 脚本、环境检查、通电日志和极简实验预告都能在同一目录下运行，才说明第一章的环境链路真正接通。</figcaption>
</figure>

提交证据不需要花哨。它像实验记录里的“原始结果”：命令跑过，输出出现，日志文件存在，下一次别人也能沿着同样路线复现。环境配置的自信，不是靠感觉来的，是靠证据一点点堆出来的。
