...
<figure align="center">
  ...
  <figcaption><strong>图10-1 </strong>：办公自动化不是让电脑替你思考，而是把重复、容易出错、需要固定格式交付的工作交给程序。</figcaption>
</figure>


> 本章一句话：CSV 是原料，Python 是流水线，Word/Excel/PPT/Markdown 是成品出口。

第 10 章进入一个具体的交付场景：把一批学习记录自动汇总成报告。最后你会得到一组真正的文件：`final_report.md`、`final_report.docx`、`final_report.xlsx`、`final_slides.pptx`。

这就是办公自动化最迷人的地方：它不炫技，但很省事。少复制一次表格，少手动改一次标题，少在凌晨两点对着 Word 目录发呆，都是人类文明的小胜利。
...
7. **生成检查预览图**：用 Pillow 将表格数据绘制成图片，方便快速检查与分享。
8. **培养可复用的自动化意识**：学会把固定格式、固定来源、固定输出的工作变成稳定、可复现的流程。

### 10.0.1 Office 函数：改了内存对象，不等于已经保存

Word、Excel 和 PPT 自动化通常分三步：创建或打开文档对象，在内存中添加内容，最后调用 `save()` 写入磁盘。前两步顺利运行却没有生成文件，往往只是漏了最后一步，或者保存到了自己没注意的路径。

| 调用 | 输入 | 返回值 | 文档或文件会怎样 |
|---|---|---|---|
| `Workbook()` | 通常无 | 新的 Excel 工作簿对象 | 只在内存中创建，还没有 `.xlsx` 文件 |
| `load_workbook(path)` | 已有 Excel 文件路径 | 读入后的工作簿对象 | 打开已有内容供修改，不会自动保存 |
| `ws.append([name, score])` | 一行单元格值 | `None` | 把一行加入内存中的工作表 |
| `workbook.save(path)` | 输出路径 | `None` | 真正创建或覆盖 Excel 文件 |
| `Document()` | 无或已有 Word 路径 | Word 文档对象 | 新建或打开内存中的文档 |
| `document.add_heading(text, level=1)` | 标题文字和级别 | 新段落对象 | 把标题加入内存文档，可继续设置格式 |
| `document.add_table(rows, cols)` | 行数和列数 | 表格对象 | 在内存文档中加入表格 |
| `Presentation()` | 无或已有 PPT 路径 | 演示文稿对象 | 新建或打开内存中的 PPT |
| `presentation.slides.add_slide(layout)` | 幻灯片版式 | 新幻灯片对象 | 在演示文稿中加入一页 |
| `Inches(1)` / `Pt(12)` | 数值 | Office 使用的长度对象 | 只做单位转换，不会自己改变文档 |
| `document.save(path)` / `presentation.save(path)` | 输出路径 | `None` | 把 Word 或 PPT 结果写到磁盘 |

所以每个自动化脚本结束前都检查三件事：保存函数是否真的执行、输出路径是不是你以为的目录、保存后的文件能否重新打开。终端打印“完成”只是提示文字，文件存在且内容正确才是最终证据。
...
| 第三部分：成品预览与脚本导览 | 10.10-10.11 | 办公自动化必须让结果看得见、查得到、能复现 |
...
上图是Margaret Hamilton 站在阿波罗导航软件代码清单旁边的照片。它提醒我们：文档不是"写完代码以后顺手补一下"的装饰品，而是可靠系统的一部分。任务越重要，越不能只靠"我记得我做过"。
...
| 6 | 预览图片 | `reports/final_report_preview.png` 和 `reports/excel_workbook_preview.png` 作为检查证据 |
...
可以按这个顺序检查。第 10 章的命令不是为了“显得高级”，而是为了留下可复现的运行证据：你要知道自己在哪个文件夹里、用了哪个 Python、生成了哪些文件。

1. 打开项目目录 `python_tutorial_ch10`。
2. 如果你在 PowerShell 里运行，先确认当前目录。假设你已经在教程总目录 `Python教程` 里，可以执行：

```powershell
cd python_tutorial_ch10
Get-Location
Get-ChildItem code\ch10
```

如果能看到 `01_make_report_data.py` 到 `05_make_excel_preview.py`，说明位置对了。后面所有命令都默认你站在 `python_tutorial_ch10` 这个目录里。

3. 进入解释器设置，选择你正在使用的 Python，例如 Python 3.11。
4. 在同一个解释器环境里安装依赖：
...
这行命令可以拆开理解：`python -m pip` 表示“用当前这个 Python 去调用 pip”，`requirements.txt` 是本章需要的第三方库清单。这样做比单独输入 `pip install ...` 更不容易装错环境。

5. 按下面的顺序运行脚本：

| 顺序 | 命令 | 目的 | 运行后应看到 |
| --- | --- | --- | --- |
| 1 | `python code\ch10\01_make_report_data.py` | 生成原料 CSV | `input\report_data.csv` |
| 2 | `python code\ch10\02_generate_markdown_report.py` | 生成轻量报告 | `reports\final_report.md` |
| 3 | `python code\ch10\03_optional_docx_hint.py` | 可选：单独演示 Word 生成 | `reports\final_report.docx` |
| 4 | `python code\ch10\04_generate_office_pack.py` | 生成 Office 成品包 | Word、Excel、PPT、报告预览图 |
| 5 | `python code\ch10\05_make_excel_preview.py` | 读取 Excel 并生成检查图 | `reports\excel_workbook_preview.png` |

脚本 3 是一个小演示，可以跳过；脚本 4 会重新生成更完整的 Word 文档。真正想拿到本章完整结果，至少要按顺序运行脚本 1、2、4、5。
...
| 预览图 | 让报告结果能直接放进教程或复盘 | `04_generate_office_pack.py`、`05_make_excel_preview.py` |
...
按顺序跑完脚本 1、2、4、5 后，`reports/` 里会出现这组文件。这里要特别分清：`final_report.md` 来自脚本 2；Word、Excel、PPT 和 `final_report_preview.png` 来自脚本 4；`excel_workbook_preview.png` 来自脚本 5。
...
运行脚本 2 前，必须已经有 `input/report_data.csv`。如果这里报“找不到文件”，先回去运行脚本 1。
...
它是本章的 Office 成品脚本，会读取脚本 1 生成的 CSV。注意它不负责生成 `final_report.md`，Markdown 报告已经由脚本 2 完成。
...
它负责把真实 Excel 工作簿变成可快速查看的图片。运行它之前，必须先跑脚本 4，因为它要读取 `reports/final_report.xlsx`。
...
- **先后顺序错了**：没有先生成 CSV，就直接生成报告；或者没有先生成 Excel，就直接生成 Excel 预览图。
- **只生成不检查**：文件有了，但内容是否正确完全没看。

遇到错误时，先读报错的第一行和最后一行。这里的信息比较凝练和关键。
