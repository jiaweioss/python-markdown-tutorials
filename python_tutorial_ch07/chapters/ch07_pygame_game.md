...
| 第四部分：排错、项目与反馈证据 | 7.8 | 小游戏不是窗口能动就结束，还要能调参、复盘和交付 | 坑地图、项目面板、调参图、运行证据 |
...
**`while running:`**——这不是只执行一次的普通循环，而是**每秒重复约 60 次的"心脏泵"**。每一轮循环做三件事：读输入 → 更新状态 → 重绘画面。只要 `running` 为 `True`，这个循环就不会停。
...
按 R 键把所有游戏状态变量重置为初始值。**重置状态 = 重新开始游戏**——这就是游戏开发中"重开"的本质：不是重新运行程序，而是把变量恢复到初始值。
...
这恰恰是本章后续脚本和报告系统的用武之地——它们会帮你把游戏过程中产生的数据记录下来，生成可以复盘和分析的报告。
