...
第7章继续推进“科研卡片工厂”的交互能力。前面几章主要处理文件、表格和报告；这一章开始，程序能对人的操作立即作出回应：按下一个键，画面和分数随之改变。对心理学和教学来说，这很重要，因为很多任务本来就是刺激、反应、反馈和记录的循环。

本章的目标很明确：做一个能讲清楚原理、也确实跑得起来的小作品。
...
### 7.0.1 PyGame 函数：有些返回状态，有些直接改变画面

游戏程序会一遍又一遍执行循环，因此函数调用的位置和返回值同样重要。事件只读取一次、画面只更新一次，和每帧都读取、每帧都重画，结果完全不同。

| 调用 | 输入 | 返回值 | 状态或画面变化 |
|---|---|---|---|
| `pygame.init()` | 无 | 成功和失败的模块数量 | 初始化 PyGame 子模块 |
| `pygame.display.set_mode((800, 600))` | 窗口宽高 | 屏幕 `Surface` 对象 | 创建游戏窗口 |
| `pygame.event.get()` | 通常无 | 本帧待处理的事件列表 | 取走事件队列中的事件，所以应集中读取一次 |
| `pygame.key.get_pressed()` | 无 | 表示每个按键状态的序列 | 读取当前键盘状态，不改变画面 |
| `pygame.draw.rect(screen, color, rect)` | 目标画布、颜色、矩形 | 绘制区域的 `Rect` | 先改内存中的画布，还未显示到屏幕 |
| `font.render(text, True, color)` | 文字、抗锯齿开关、颜色 | 包含文字的新 `Surface` | 不会自动把文字放到窗口中 |
| `screen.blit(surface, position)` | 要贴上的图面和位置 | 受影响的矩形区域 | 把内容画到内存中的屏幕表面 |
| `pygame.display.flip()` | 无 | `None` | 把这一帧真正显示出来 |
| `clock.tick(60)` | 目标最高帧率 | 距上一帧经过的毫秒数 | 必要时等待，避免循环跑得过快 |
| `pygame.quit()` | 无 | `None` | 关闭已经初始化的 PyGame 模块 |

可以把一帧记成四步：**收事件 -> 改状态 -> 画画面 -> 显示画面**。函数放错阶段时，程序不一定报错，却可能出现按键没反应、文字不显示或速度失控，这也是游戏代码比普通脚本更需要按流程阅读的原因。
...
很多人一听到“游戏”，会先想到“玩物丧志”。但在心理学和教学领域，游戏也可以是实验任务、反馈系统和练习场。

初学者最怕一上来就被术语包围，像走进一栋所有门牌都用缩写写成的楼。我们先从画面进入，再慢慢把画面翻译成代码。
