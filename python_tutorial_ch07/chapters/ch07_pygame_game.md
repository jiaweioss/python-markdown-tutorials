...
5. 运行本章非交互脚本，生成反应报告、难度平衡图、反馈循环卡、心流曲线、数据驱动调参和运行记录。
...
| 第四部分：排错、项目与反馈记录 | 7.8-7.9 | 小游戏不是窗口能动就结束，还要能调参、复盘和成果 | 坑地图、项目面板、调参图、运行记录 |
...
PyGame 可以把这个原则做得很具体。屏幕上出现一张卡片，你按下数字键，程序立刻显示“答对”“再试一次”或“先看提示”，同时把反应时写进文件。这个过程一点也不神秘：事件循环负责读按键，状态变量负责记当前卡片，绘图函数负责反馈，文件负责留下记录。所谓教学小游戏，不是给语法穿上花衣服，而是让学习者和程序之间真的有来有回。
...
<figure align="center">
  <img src="../assets/ch07/ch07_data_driven_tuning.png" alt="Python 生成的数据驱动游戏调参记录" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图7-22 Python 生成的数据驱动游戏调参记录</strong>：`08_make_data_driven_tuning.py` 读取 ch6 的分析摘要，把卡片数量、标签数量和反应时转换成出题间隔、目标反应窗口和一轮卡片数。</figcaption>
</figure>
...
最后，再给本章做一次“游戏记录检查”。小游戏最容易让人误以为“窗口能打开就完事”，但真正可复盘的学习工具，还应该留下报告、调参数据和反馈计划。下面这张图把本章的 14 个关键产物放在同一块记录板上：反应报告、难度平衡、反馈循环、心流曲线、数据驱动调参和教学反馈小游戏都要 ready。
...
<figure align="center">
  <img src="../assets/ch07/ch07_game_runtime_evidence.png" alt="PyGame游戏运行记录总览" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图7-25 PyGame游戏运行记录总览</strong>：`10_make_game_runtime_evidence.py` 检查本章 14 个关键产物，把“小游戏能运行”升级成“窗口、报告、调参和反馈计划都能被复盘”。</figcaption>
</figure>
...
11. **生成运行记录**：运行 `10_make_game_runtime_evidence.py`，确认本章关键游戏产物全部 ready。
...
10. 运行 `10_make_game_runtime_evidence.py`，如果报告里出现“缺失”，根据缺失路径回头补跑对应脚本。
