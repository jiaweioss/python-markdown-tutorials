...
### 9.0.1 图像函数：返回新图，还是修改原图？

Pillow 的方法看起来都像 `image.xxx(...)`，行为却不完全一样。最重要的判断是：调用以后应不应该用变量接住返回值。没有接住新图，后面保存的可能仍是原图；误把原图直接保存到同一路径，又可能再也找不回未经处理的版本。

| 调用 | 输入 | 返回值 | 原图或文件会怎样 |
|---|---|---|---|
| `Image.open(path)` | 图片路径 | 延迟读取的图片对象 | 打开文件供读取，不会修改原图 |
| `Image.new("RGB", (800, 600), "white")` | 模式、尺寸、底色 | 新图片对象 | 只存在内存中 |
| `image.convert("RGB")` | 目标色彩模式 | 转换后的新图片 | 原 `image` 不变，应接住返回值 |
| `image.resize((400, 300))` | 新宽高 | 缩放后的新图片 | 原图不变；直接给固定宽高可能拉伸 |
| `image.crop((left, top, right, bottom))` | 裁剪边界框 | 裁剪后的新图片 | 原图不变 |
| `image.filter(ImageFilter.BLUR)` | 滤镜对象 | 应用滤镜后的新图片 | 原图不变 |
| `image.paste(other, position)` | 另一张图和粘贴位置 | 通常返回 `None` | 直接修改当前 `image` 对象 |
| `draw.text(position, text, font=...)` | 坐标、文字和字体等 | 通常返回 `None` | 直接画到关联的图片对象上 |
| `image.save(output_path)` | 输出路径和可选格式 | 通常返回 `None` | 创建或覆盖磁盘文件 |

新手可以用一个简单规则自查：转换、缩放、裁剪和滤镜通常写成 `new_image = image.xxx(...)`；粘贴和绘制通常直接改当前图片；只有 `save()` 才把内存结果写到磁盘。保存时优先使用新的文件名，确认效果后再决定是否替换原图。
