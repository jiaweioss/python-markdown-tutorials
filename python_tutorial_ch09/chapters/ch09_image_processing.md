...
1. 用“像素、坐标、颜色通道、处理动作、学习记录链”解释图像处理的最小工作链路。
...
| 第二部分：像素、素材和最小示例 | 9.3-9.4 | 把图片看成坐标和颜色通道，再用最小脚本跑通处理动作 | 核心比喻、真实照片素材、处理前后图、运行记录 |
| 第三部分：感知、科研图片和概念表 | 9.5-9.6 | 程序改的是像素，人理解的是场景；处理要服务于理解和记录 | 心理学连接、Helmholtz、棋盘阴影错觉、概念表 |
| 第四部分：脚本、排错和项目成果 | 9.7-9.9 | 每个脚本都要留下输入、处理、输出和可复盘结果 | 脚本清单、坑地图、项目面板、故事板、视觉记录档案 |
...
- **Pillow 打开保存**：先保留原图，再生成副本；不要让一次实验覆盖记录。
...
图像处理不怕步骤多，怕的是只留下一个“最终版”，却说不清它从哪里来。下面这张运行记录图把本章关键产物排成清单：原始卡片、灰度图、真实照片处理、视觉实验、质量总览、ch8 素材入库、处理故事板和视觉记录档案都要能被检查到。

<figure align="center">
  <img src="../assets/ch09/ch09_image_runtime_evidence.png" alt="PowerShell 风格的图像处理运行记录总览" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图9-13 PowerShell 风格的图像处理运行记录</strong>：`11_make_image_runtime_evidence.py` 检查本章关键图片和报告是否全部生成，让“我跑过了”变成可复盘、可复查的记录。</figcaption>
</figure>
...
这张图像是在给图像处理“泼一杯清醒水”。Python 看到的是像素值，人看到的是场景。你把亮度调高一点，程序觉得只是数值变化；观看者可能会觉得“依据更清楚了”。你裁掉一个边角，程序觉得只是坐标变了；别人可能会失去判断上下文。图像处理越强大，越要诚实。
...
它的作用不是再制造一张“好看的图”，而是把原始卡片、灰度版本、真实照片处理、知觉实验台、质量总览、处理故事板和 ch8 素材入库结果放到一起。做图像处理时，最怕只留下一个最终文件，却说不清它从哪里来、经过了什么处理、有没有被过度裁剪。记录档案就是给每张图留下来龙去脉。如果你运行了下面的 `10_make_processing_storyboard.py`，可以再回头运行一次这个脚本，让档案刷新到最新状态。
...
它训练的是图像处理里最重要的作品意识：每一步都要留下文件，每一步都要能解释目的。原图负责保留记录，灰度负责检查明暗，裁剪负责选择焦点，增强负责让细节更清楚，卡片版负责最终成果。处理图片不是在滤镜商店里随便试衣服，而是在给一个真实任务做合适的版式。
...
这个脚本会检查本章关键输出是否都已经生成，并整理成一张 PowerShell 风格的运行记录图：
...
它的作用像一张“图像处理检查单”：不是继续加工图片，而是确认加工结果、报告、故事板和记录档案都在正确位置。以后做课程分享或科研报告时，这一步很有用，因为它把“我记得跑过”变成“我能拿出记录”。
...
这张故事板把图像处理从“我调了几个参数”变成“我完成了一条成果路径”。原图不要删，因为它是记录；灰度不是退回黑白电视，而是帮你检查明暗结构；裁剪不是随手切一刀，而是决定观看者先看哪里；增强不是越猛越好，而是让关键信息更清楚；卡片成品才是最后要进入科研卡片工厂的材料。
...
<figure align="center">
  <img src="../assets/ch09/ch09_visual_evidence_archive.png" alt="Python 生成的视觉记录档案" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图9-24 Python 生成的视觉记录档案</strong>：`09_make_visual_evidence_archive.py` 把本章关键输出集中成一张可复盘总览，方便检查原图、处理图、处理故事板、质量总览和跨章节素材入库结果。</figcaption>
</figure>

这张图是本章的“记录桌面”。想象你正在给一组科研图片做整理：如果只留下最终版，过几天你很可能会忘记它是怎么裁的、为什么变灰、有没有过度锐化、原图是否还在。视觉记录档案把这些产物摆在一起，让你能顺着路径追回去。
...
1. 能按顺序运行 `01_create_demo_image.py` 到 `10_make_processing_storyboard.py`，重新运行 `09_make_visual_evidence_archive.py` 刷新记录档案，再运行 `11_make_image_runtime_evidence.py` 生成运行记录。
...
9. 能生成 `reports/ch09_visual_evidence_archive.md` 和 `output/visual_evidence_archive.png`，并解释为什么图像处理要保留学习记录链。
...
10. **刷新记录档案**：运行 `09_make_visual_evidence_archive.py`，把关键输出集中成可复盘总览。
11. **生成运行记录**：运行 `11_make_image_runtime_evidence.py`，检查本章关键产物是否全部落盘。
...
9. 运行 `09_make_visual_evidence_archive.py`，把自己新增的处理结果也放进视觉记录档案，并说明它解决了什么问题。
