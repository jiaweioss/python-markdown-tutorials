...
def write_storyboard() -> None:
    ...
    steps = [
        ("Open", "window", card_form, BLUE),
        ("Type", "input", simple_window("type"), PURPLE),
        ("Click", "button", simple_window("click"), ORANGE),
        ("Feedback", "state", simple_window("feedback"), GREEN),
        ("File", "saved", delivery, RED),
    ]
    image = Image.new("RGB", (1720, 980), "#F7F8FB")
    draw = ImageDraw.Draw(image)
    draw.text((100, 72), "GUI Journey Storyboard", fill=INK, font=font(56, True))
    draw.text((100, 140), "Open  Type  Click  Feedback  File", fill=MUTED, font=font(25))
    ...
    draw.text((710, 736), "input -> action -> result", fill=INK, font=font(23, True))
    draw.text((600, 875), "Generated by code/ch04/09_make_gui_journey_storyboard.py", fill=MUTED, font=font(20))
