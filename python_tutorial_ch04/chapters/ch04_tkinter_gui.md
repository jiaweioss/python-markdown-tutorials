...
第4章给“科研卡片工厂”装一个前台入口。前面几章已经处理好环境、数据类型和文件管理；现在要让用户不必记住 `Path.write_text()`，也不必知道脚本里的函数名，只要填内容、点按钮、看到反馈，就能在 `cards/` 或 `reports/` 里找到结果。

本章的读法和第1章保持一致：**先跑真实脚本，再看截图证据，最后把概念拆成能复查的动作**。先别一上来背 Tkinter API。先让一个窗口真的停在屏幕上，再逐步问：窗口是谁创建的，控件从哪里来，按钮点下去之后到底调用了哪个函数，保存结果在哪里。
...
### 4.0.1 第一次看 Tkinter：括号里的参数会变成界面

Tkinter 代码里既有“创建对象”的调用，也有“让对象做事”的方法。先别急着背参数：每看到一对括号，先问它接收什么、返回什么，以及窗口是否因此发生变化。

| 调用 | 主要输入 | 输出或返回值 | 界面会发生什么 |
|---|---|---|---|
| `root = tk.Tk()` | 通常不传参数 | 一个主窗口对象，保存到 `root` | 创建应用的根窗口，但还没有进入事件循环 |
| `tk.Label(root, text="姓名")` | 父容器 `root`、显示文字等选项 | 一个标签控件对象 | 控件被创建；还要布局后才会出现在窗口里 |
| `widget.pack()` / `widget.grid(...)` | 间距、行列位置等布局参数 | 通常返回 `None` | 修改控件的布局状态，让它出现在窗口中 |
| `entry.get()` | 不需要额外参数 | 输入框当前文字，类型是 `str` | 只读取内容，不会清空输入框 |
| `widget.config(text="已保存")` | 要更新的属性 | 通常返回 `None` | 直接修改已有控件的显示状态 |
| `root.bind("<Escape>", close_window)` | 事件名称、回调函数 | 一个绑定标识或空字符串 | 以后按下对应按键时调用回调 |
| `messagebox.showinfo("完成", "保存成功")` | 标题和提示文字 | 用户关闭对话框后的结果 | 弹出一个消息窗口 |
| `root.mainloop()` | 无 | 直到窗口关闭才结束 | 持续等待鼠标和键盘事件；通常放在程序最后 |

这里最反直觉的是 `command=save_card`：它把“函数本身”交给按钮，等用户点击时再调用。若写成 `command=save_card()`，括号会让函数在创建按钮时立刻执行，按钮真正被点击时反而没有函数可用。你会在本章后面看到完整例子。
...
Tkinter 控件看起来像零件表，但它们要服务同一个任务。我们先学习最常用的四个：
...
控件本身不等于界面好用。`Entry` 如果旁边没有标签，用户不知道该填什么；`Button` 如果只写“确定”，用户不知道确定什么。对于初学者来说，GUI 设计的重点是把任务说清楚。

早期图形界面之所以能降低陌生感，很大一部分靠“可识别的符号”。Susan Kare 为 Macintosh 设计的图标之所以经典，不只是像素画漂亮，而是把“保存、文件夹、警告、垃圾桶”这些抽象动作变成用户一眼能猜到的形状。我们写 Tkinter 时没有必要做整套图标，但要学会同一种态度：控件文字、按钮位置和反馈文案都要帮用户少猜。
...
这行看着很像正确写法，但它会在创建按钮时立刻执行 `save_card()`。按钮还没被用户点击，函数已经运行了。正确写法是：
...
这不仅仅是语法小细节，而且是 GUI 程序的核心思路。命令行脚本通常从上到下依次执行，所有动作立刻发生。GUI 程序则不同：大部分时间它只是“等着”，等用户点按钮、按键盘、关闭窗口，才去执行对应的函数。这种“等事件发生再响应”的模式，就叫**事件驱动**。`command=save_card` 就是把函数注册为一个事件的响应动作——函数现在不跑，但按钮被点击的那一刻它会跑。
