...
这不仅仅是语法小细节，而且是 GUI 程序的核心思路。命令行脚本通常从上到下一次执行，所有动作立刻发生。GUI 程序则不同：大部分时间它只是"等着"，等用户点按钮、按键盘、关闭窗口，才去执行对应的函数。这种"等事件发生再响应"的模式，就叫**事件驱动**。`command=save_card` 就是把函数注册为一个事件的响应动作——函数现在不跑，但按钮被点击的那一刻它会跑。
