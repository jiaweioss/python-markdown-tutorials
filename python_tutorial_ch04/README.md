...
- 正文重排为对标 ch01 的结构：本章导读、学习目标、分区导航、五个部分、上机路线、提交证据、自测问题和复盘模板。
- 正文保留 26 张有明确教学作用的配图，删去旧稿中成串的无信息量图标拼贴；GUI 历史图片重新贴着概念穿插，服务“输入设备、窗口、图标、可用性、步骤、真实截图、交付证据”。
- 重写 `scripts/generate_ch04_visuals.py`，用可复现的 Pillow 绘图生成结构化步骤图，支持中文自动换行与字号适配，避免文字越界。
- 把泛化讲解改成具体 Tkinter 心智模型：`Tk()`、控件、布局、回调、`mainloop()`、反馈和文件证据。
...
- 修正文口吻：减少模板句和工具腔表达，把每节内容落到学生能直接运行、检查、交付的 GUI 小任务。
