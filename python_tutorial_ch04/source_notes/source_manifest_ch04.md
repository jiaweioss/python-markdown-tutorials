...
本章保留 26 张可由 `scripts/generate_ch04_visuals.py` 生成或整理的正式图片资产，其中正文当前引用 20 张。旧版无信息量的图标拼贴已经替换为结构化步骤图，包括封面、学习路线、命令行到 GUI 的转换、Tkinter 心智模型、最小窗口代码映射、心理学实验流程、常见坑排查和项目面板图。
