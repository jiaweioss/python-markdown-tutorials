...
| 有明确章节分界 | 正文拆成五个部分：窗口通电、控件表单、事件反馈、项目交付、排错验收 | 已完成 |
| 图片服务教学动作 | 正文引用 20 张图，分别对应路线、步骤、代码映射、真实窗口、表单、反馈、交付、排错和运行证据 | 已完成 |
| 去掉无意义图标拼贴 | 旧版核心图、封面、路线图、常见坑图、项目图等已由结构化步骤图替换 | 已完成 |
| 图片文字不越界 | 新版 `generate_ch04_visuals.py` 使用中文换行和字号适配；抽查路线图、最小窗口图、可用性图无溢出 | 已完成 |
| 真实运行证据 | 保留 3 张 Tkinter 窗口截图，并保留 GUI 可用性、反馈检查、交互回执、卡片交付、旅程图和运行证据图 | 已完成 |
| 学生阅读口吻 | 旧稿重复模板句已改为“运行脚本、看截图、查证据、完成项目”的直接任务口吻 | 已完成 |
...
- 重写 `chapters/ch04_tkinter_gui.md`，对齐 ch01 的导读、分区导航、证据链和复盘结构。
...
- `scripts/check_links.py`：通过，检查 20 个本地图片引用，0 个缺失链接。
