...
| 需求 | 当前记录 | 状态 |
| --- | --- | --- |
| 对标 ch01/ch04 图文标准 | 正文重写为 `[TOC]`、本章导读、分区导航、五个学习部分、运行记录、练习、自测和复盘模板 | 已完成 |
| 图片有明确教学意义 | 正文 26 张图片覆盖路线图、代码对照图、心智模型、对象协作图、质量记录、成果包、运行记录，以及 Simula、Smalltalk、认知图式和代码文化背景 | 已完成 |
| 图片居中且有图注 | 正文 26 张图片全部使用 `<figure>` 与 `<figcaption>` | 已完成 |
| 图片文字不越界 | 重建生成器后检查接触表，并满尺寸检查最小类、质量记录、对象协作图和运行记录图 | 已完成 |
| 学生阅读口吻 | 删除旧模板里的历史堆叙述和机械说明，改为“先跑脚本、再看记录、最后划清职责”的学习路线 | 已完成 |
| 真实运行环境 | `ch05_powershell_oop_run.png` 与 `ch05_oop_runtime_evidence.png` 展示本章脚本运行和成果文件就绪状态 | 已完成 |
| 学以致用 | 本章项目为“学习卡片对象模型”，包含对象模型报告、类职责卡片、对象协作消息图、对象质量记录、对象成果包和 ch4 GUI 面板对象模型 | 已完成 |
