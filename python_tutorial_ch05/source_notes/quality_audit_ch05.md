...
| 图片有明确教学意义 | 正文 17 张图片均为路线图、代码对照图、心智模型、对象协作图、质量回执、交付包或运行证据；历史照片保留在资产目录但不再正文堆放 | 已完成 |
| 图片居中且有图注 | 正文 17 张图片全部使用 `<figure>` 与 `<figcaption>` | 已完成 |
...
- 运行 `python scripts/check_links.py`，确认 Markdown 图片链接有效，检查 17 个本地图片引用。
