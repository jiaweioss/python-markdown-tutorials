...
> 本章一句话：  
> **面向对象不是把代码写得更“高级”，而是给数据和动作划清边界。一个好对象知道自己保存什么、能做什么、不该管什么，也清楚该把请求交给哪个对象。**
...
对象不是另一套神秘语法。`类名(...)` 是创建对象，`对象.方法(...)` 是请这个对象完成一项工作。方法定义里的 `self` 代表当前对象；调用 `card.review()` 时，Python 会自动把 `card` 交给 `self`，所以括号里不用再写一遍。

| 写法 | 传入什么 | 得到什么 | 原对象会不会变 |
|---|---|---|---|
| `LearningCard("循环", "重复执行")` | 创建对象需要的初始数据 | 一个新的 `LearningCard` 对象 | 创建新对象，不修改其他对象 |
| `card.answer` | 不使用括号；这是读取属性 | 属性中保存的值 | 不会改变对象 |
| `card.review(correct=True)` | 本次复习结果 | 由方法中的 `return` 决定；没有 `return` 时是 `None` | 可能更新次数、分数等属性 |
| `deck.add(card)` | 一张卡片对象 | 通常返回 `None` | 会把卡片加入 `deck` 内部的列表 |
| `deck.summary()` | 通常不需要参数 | 汇总字符串或字典 | 一般只读取牌组状态 |
| `@dataclass` | 写在类定义上，不是普通运行步骤 | 自动生成初始化等方法 | 改变类的定义方式，让数据类少写重复代码 |
| `field(default_factory=list)` | 一个负责创建默认值的函数 | 每个对象各自使用的新列表 | 避免多个对象错误地共用同一个列表 |

判断一个方法时，不要只看名字。请顺着方法体找两处：`return` 告诉你调用表达式会得到什么，`self.xxx = ...` 或 `self.xxx.append(...)` 告诉你对象内部哪些状态被改变。两者可以同时发生，也可以都不发生。
...
把“同一件事”收进一个边界，这个思路在心理学里也能找到呼应。人理解世界时，会把分散信息组织成可重复使用的结构，Piaget 把这种结构叫作“图式”（schema）。把一次试次收成一个对象，和这个道理有一点相通。Piaget 研究儿童认知发展时关心的正是这种结构如何形成、调整和迁移。写代码当然不是照搬心理学理论，但这个类比能提醒我们：好对象不是一堆字段，而是一个有边界、有意义、能参与后续动作的整体单元。
