...
<figure align="center">
  ...
  <figcaption><strong>图6-1 第6章封面</strong>：本章把学习记录变成表格、摘要、图表和下一步行动。</figcaption>
</figure>

> 本章一句话：
> **数据分析的核心是用一条可复查的链路回答问题：数据从哪里来，算出了什么，图表说明什么，下一步该做什么。**

前面几章，我们已经学会写函数、处理文件、构建 GUI，还在第5章把学习卡片、试次记录和交付包整理成了对象。到了第6章，项目进入"看证据"的阶段：同样是一组学习记录，放在 CSV 里还只是原材料——经过统计、可视化和复盘，才能变成指导下一次学习的依据。

在这一章，我们只抓住一条主线：**先提出一个足够具体的问题，再用 Python 留下可复查的输出。** 你会生成 `learning_records.csv`，计算平均时长和完成率，画出学习仪表盘，比较 Anscombe 四重奏，诊断异常值，把第5章的对象交付包读进来，最后生成一份运行证据总览。
...
### 6.0.1 数据函数：先分清读入、计算和写出

数据分析代码常把多个函数连在一起。新手最容易把“屏幕上显示了结果”和“变量里真的得到结果”混为一谈。下面先把本章常见调用按数据流拆开：文件进入内存，内存中的数据被计算，最后再选择是否写回文件。

| 调用 | 输入 | 返回值 | 文件或数据会怎样 |
|---|---|---|---|
| `csv.DictReader(file)` | 已打开的文本文件 | 可逐行读取的对象；每行像一个字典 | 只读文件，不会自动把全部内容变成列表 |
| `pd.read_csv(path)` | CSV 路径和可选的编码等参数 | 一个 `DataFrame` | 把文件内容读进内存，不修改原 CSV |
| `df.groupby("topic")` | 分组所依据的列名 | 一个分组对象 | 暂时描述如何分组，还没有生成最终统计表 |
| `series.mean()` | 通常不传参数 | 平均值 | 不修改原数据 |
| `series.median()` | 通常不传参数 | 中位数 | 不修改原数据；它和平均值回答的问题不同 |
| `df.to_csv(path, index=False)` | 输出路径、是否保存行索引 | 通常返回 `None` | 在磁盘创建或覆盖 CSV 文件 |
| `Image.new("RGB", size, color)` | 色彩模式、尺寸、背景色 | 一张新的 Pillow 图片对象 | 只在内存中创建，尚未保存 |
| `image.save(path)` | 输出路径 | 通常返回 `None` | 真正把图片写到磁盘 |

如果看到 `result = ...`，说明程序打算保留返回值继续使用；如果只写 `df.to_csv(...)` 或 `image.save(...)`，重点通常是它造成的文件变化。运行后记得同时检查终端信息和输出目录，两处都是证据。
...
这些问题不需要宏大的数据集。三行学习记录就足够练习完整流程，关键在于让“输入、处理、输出、解释”形成闭环。
...
需要说明一下：这里的 `rt_ms` 不是心理学实验里的严格测量，它只是本章项目中用来反映学习感受的一个线索。初学阶段，把概念说清楚比套用复杂模型更重要。
...
你应该看到记录数、平均学习时长、平均反应时和完成率。`03_optional_pandas_summary.py` 会尝试使用 pandas；如果你的环境里没有 pandas，可以用之前学过的办法安装它。
...
<figure align="center">
  ...
  <figcaption><strong>图6-5 第6章脚本运行证据</strong>：脚本实打实地生成 CSV、统计输出、图表、报告和运行证据，而不只是在正文里展示概念。</figcaption>
</figure>
...
介绍数据分析时，人们很容易列出一堆工具名字：pandas、NumPy、Matplotlib、Seaborn、Excel、Jupyter。工具当然重要，但初学阶段不妨先记住下面这条链：
...
William Playfair 早期把价格、工资和时间画成曲线，做的事情也一样：本来是逐行读表格，变成了一眼就能看到变化的轨迹。你在学习仪表盘里画柱状图、折线图，本质上也一样——让时间、差异和转折露出来。
...
<figure align="center">
  ...
  <figcaption><strong>图6-S3 Playfair 的时间序列图</strong>：折线图真正表达的是让变化从表格里走出来。</figcaption>
</figure>
...
脚本会读取 CSV，生成 `output/ch06_learning_dashboard.png`，并同步一份网页用图到 `assets/ch06/web/ch06_learning_dashboard_output.png`（教材里的正式插图就是用它排版的）。
...
这是一个简洁的 CSV 读取写法：`csv.DictReader(f)` 把文件的每一行解析为一个字典，字典的键是 CSV 第一行的列名（`topic`、`minutes`、`done`、`rt_ms`）。外面套一层 `list()` 把迭代器转为列表，便于后续多次遍历。`encoding="utf-8"` 确保中文列名和内容不会出现乱码。
...
这是 Python 脚本中常见的**入口写法**。`__name__` 在直接运行脚本时为 `"__main__"`，在被其他模块导入时则为模块名。这行代码确保 `main()` 只在直接运行时执行，不会在导入时意外触发。
...
1. **用 Pillow 绘制图表是"像素级"操作**：每个文字、每个矩形、每个圆的位置都需要手动计算坐标。这和 Excel 或 Matplotlib 的"声明式"画图完全不同——Pillow 给你最大的自由度，也要求你做手动计算。
...
这张图只想回答一个问题：**哪几个主题占用了更多学习时间？** 回答这个问题不需要散点图、堆叠图或复杂的坐标系——几条彩色条形就够了。学习画图的第一课是"忍住不画多余的东西"。
...
Anscombe 想用这个例子说明一件事：**先算再画还不够，画了才算。** 统计数字会压缩信息，而图形能把那些被压缩掉的细节重新展现出来。从此，"先画图"成了数据分析入门的第一课。
...
Anscombe 四重奏像一个很温柔但很锋利的提醒：统计摘要会压缩信息，压缩就会丢东西。它的本意是提醒我们给统计配上图形检查，而不是反对统计本身。只看平均数，就像只看一本书的目录；目录有用，但不能替你读完整故事。
...
Minard 的拿破仑远征俄国图和 W.E.B. Du Bois 的数据肖像，恰好用来说明：图表审美不是装饰，它和表达立场、降低混乱、组织变量有关。一张好图可能同时有路线、人数、温度、时间，也可能用大胆构图展示社会处境；关键在于让读者知道该沿着哪条线索读，而不是一味追求复杂。
...
<figure align="center">
  ...
  <figcaption><strong>图6-S6 Du Bois 的数据肖像</strong>：数据图表也有人文立场；颜色、构图和标题都在帮你想清楚“要回答什么问题”。</figcaption>
</figure>

图表审美不是“好不好看”这么简单。对学习项目来说，审美的本质是减少误读：让读者更快看见你想比较的东西，也方便追问数据从哪里来。
...
异常值怎么处理的，最好记下来。在文档里写下你的判断：它是错误、困难，还是特殊安排。
...
不要一出错就重写整章代码。数据分析项目最怕盲目动手。先定位是哪一环断了，再修那一环。
...
第6章把“会写代码”推进到“能用代码看证据”。学完本章，你已经完成了从 CSV 到统计摘要、从摘要到图表的闭环。
