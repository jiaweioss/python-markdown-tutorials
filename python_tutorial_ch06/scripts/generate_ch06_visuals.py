...
VISUALS = ['ch06_cover.png', 'ch06_story_scene.png', 'ch06_roadmap.png', 'ch06_core_metaphor.png', 'ch06_minimal_demo.png', 'ch06_psychology_link.png', 'ch06_pitfall_map.png', 'ch06_project_dashboard.png']
...
def data_detective_desk():
    im, d = canvas()
    shadow(d, (120, 95, 1680, 1005), radius=34)
    rounded(d, (120, 95, 1680, 1005), fill="#FFFFFF", radius=34)

    d.rounded_rectangle((190, 720, 1610, 930), radius=32, fill="#E9EEF6", outline="#D8E0EC", width=2)
    d.rounded_rectangle((255, 210, 650, 650), radius=22, fill="#FFF8E8", outline="#D6B882", width=5)
    for y in (290, 355, 420, 485, 550):
        d.line((300, y, 600, y), fill="#E8D6B2", width=5)
    d.polygon([(560, 210), (650, 210), (650, 300)], fill="#F1D8A8")

    d.rounded_rectangle((780, 230, 1230, 635), radius=26, fill="#F8FAFC", outline="#CBD5E1", width=5)
    d.line((830, 560, 1180, 560), fill="#64748B", width=6)
    for i, h in enumerate([95, 155, 125, 220]):
        x = 860 + i * 78
        d.rounded_rectangle((x, 560 - h, x + 46, 560), radius=12, fill=COLORS[i % len(COLORS)])
    d.line((1085, 380, 1160, 315), fill=RED, width=10)
    d.ellipse((1145, 300, 1190, 345), fill=RED)

    d.ellipse((1190, 560, 1435, 805), outline="#334155", width=18)
    d.ellipse((1235, 605, 1390, 760), outline="#94A3B8", width=8)
    d.line((1375, 745, 1510, 875), fill="#334155", width=22)
    d.ellipse((1314, 684, 1344, 714), fill=BLUE)

    d.rounded_rectangle((320, 705, 760, 880), radius=24, fill="#F8FAFC", outline="#CBD5E1", width=4)
    for i, color in enumerate([BLUE, GREEN, ORANGE, PURPLE]):
        y = 750 + i * 30
        d.ellipse((360, y - 9, 378, y + 9), fill=color)
        d.rounded_rectangle((405, y - 8, 700, y + 8), radius=8, fill="#DDE6F2")
        d.rounded_rectangle((405, y - 8, 470 + i * 42, y + 8), radius=8, fill=color)

    for x, y, color in [(690, 330, BLUE), (760, 705, GREEN), (1240, 355, ORANGE), (1130, 830, PURPLE)]:
        d.line((900, 690, x, y), fill="#CBD5E1", width=4)
        d.ellipse((x - 16, y - 16, x + 16, y + 16), fill=color)
    save(im, "ch06_data_detective_desk.png")
