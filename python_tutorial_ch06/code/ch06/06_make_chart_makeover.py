...
def draw_bad_chart(draw: ImageDraw.ImageDraw, box, rows: list[dict[str, str]]) -> None:
    ...
    draw.text((x1 + 36, y1 + 30), "Before", fill="#9F1239", font=font(32, True))
    ...
    for i, row in enumerate(rows):
        ...
        draw.text((x + 5, base - h - 32), row["minutes"] + "min", fill="#111827", font=font(22, True))
...
def draw_good_chart(draw: ImageDraw.ImageDraw, box, rows: list[dict[str, str]]) -> None:
    ...
    draw.text((x1 + 36, y1 + 30), "After", fill="#166534", font=font(32, True))
    ...
    for i, row in enumerate(rows):
        ...
        draw.text((x + 14, base - h - 34), f"{value} min", fill="#162033", font=font(22, True))
    ...
    draw.text((x2 - 236, y_avg + 15), f"avg {avg:.1f} min", fill="#9A5A00", font=font(18, True))


def write_makeover(rows: list[dict[str, str]]) -> None:
    image = Image.new("RGB", (1600, 940), "#F7F8FB")
    draw = ImageDraw.Draw(image)
    draw.text((90, 62), "Chart Makeover", fill="#162033", font=font(50, True))
...
def write_audit_preview() -> None:
    ...
    draw.text((150, 125), "Visual Audit Checklist", fill="#162033", font=font(50, True))
    ...
    draw.text((340, 804), "Report: output/ch06_visual_check.md", fill="#28517A", font=font(22))
...
def main() -> None:
    ...
    write_audit_preview()
    copy_assets()
    print(f"已生成 {MAKEOVER}")
    print(f"已生成 {CHECK_MD}")
    print(f"已生成 {CHECK_PREVIEW}")
