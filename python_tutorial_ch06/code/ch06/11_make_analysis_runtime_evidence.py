...
CHECKS = [
    ("sample CSV", "input/learning_records.csv"),
    ("dashboard chart", "output/ch06_learning_dashboard.png"),
    ("Anscombe chart", "output/ch06_anscombe_quartet.png"),
    ("chart makeover", "output/ch06_chart_makeover.png"),
    ("visual checklist", "output/ch06_visual_check.md"),
    ("checklist preview", "output/ch06_visual_check_preview.png"),
    ("outlier report", "reports/ch06_outlier_diagnosis.md"),
    ("outlier chart", "output/ch06_outlier_diagnosis.png"),
    ("handoff report", "reports/ch06_ch05_handoff_analysis.md"),
    ("handoff JSON", "output/ch06_ch05_handoff_summary.json"),
    ("handoff chart", "output/ch06_ch05_handoff_analysis.png"),
    ("memory report", "reports/ch06_memory_review_plan.md"),
    ("memory JSON", "output/ch06_memory_review_plan.json"),
    ("memory chart", "output/ch06_memory_review_plan.png"),
    ("style clinic report", "reports/ch06_chart_style_clinic.md"),
    ("style clinic chart", "output/ch06_chart_style_clinic.png"),
]
...
def draw_image(path: Path, rows: list[dict[str, str | int | bool]]) -> None:
    ...
    draw.text((92, 70), "Data analysis runtime evidence", font=title, fill=ink)
    draw.text(
        (96, 144),
        "CSV, charts, reports, JSON files, and handoff results are checked in one place.",
        font=subtitle,
        fill=muted,
    )
    ...
    draw.text((928, 356), "A chart tells a story only after its source files can be reopened.", font=body, fill=muted)
    ...
    draw.text(
        (140, 1033),
        "Rule of thumb: beautiful charts are useful only when data, code, and reports can be checked again.",
        font=body,
        fill="#8A5A00",
    )
...
def main() -> None:
    ...
    ready = sum(1 for row in rows if row["exists"])
    print(f"ch06 analysis runtime evidence: {ready}/{len(rows)} ready")
