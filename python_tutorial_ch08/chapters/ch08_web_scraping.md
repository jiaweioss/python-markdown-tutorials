...
这一章不求“抓得多”，只求“抓得对”。初学爬虫容易过度兴奋，爬取过多的信息。就像第一次走进大型图书馆，恨不得把所有书本搬回家。但稳妥的做法是：先看目录和借阅规则，再挑那些自己确实需要的书来拿。
...
2. 运行 `01_local_html_parser.py`，先从本地 HTML 中提取链接，避免一开始就对真实网站进行采集。
3. 运行 `02_fetch_python_homepage.py`，理解为什么采集前要先看 `robots.txt` 和请求边界。
4. 运行本章脚本，生成链接 CSV、采集报告和来源卡片。
5. 编写并运行自己的爬虫脚本 `06_my_scraper.py`，理解从请求网页到保存结果的全链路。
6. 能结合常见误区排查爬虫错误，解释你的脚本避开了哪些坑。

### 8.0.1 网络函数：请求、响应和网页文字不是同一个东西

浏览器替我们隐藏了很多步骤，代码不会。爬虫需要先准备请求，再得到响应，接着读取字节、解码成文字，最后解析 HTML。中间任何一步失败，后面的变量就不会出现。

| 调用 | 输入 | 返回值 | 外部影响 |
|---|---|---|---|
| `Request(url, headers=...)` | 网址、请求头等信息 | 一个请求对象 | 只准备请求，此时还没有访问网站 |
| `urlopen(request, timeout=10)` | 请求对象、超时秒数 | 一个响应对象 | 真正发起网络访问，可能超时或被拒绝 |
| `response.read()` | 无 | 原始字节 `bytes` | 消耗响应中的数据流 |
| `data.decode("utf-8")` | 字节采用的字符编码 | Python 字符串 `str` | 不访问网络，只把字节解释为文字 |
| `parser.feed(html_text)` | HTML 字符串 | 通常返回 `None` | 让解析器逐步读取内容，并更新其内部结果 |
| `urlparse(url)` | 完整网址 | 拆分后的网址对象 | 不访问网站，只分析网址结构 |
| `writer.writerow(row)` | 一行字典或序列 | 通常返回底层写入结果 | 把一行内容写入已打开的 CSV 文件 |
| `time.sleep(1)` | 暂停秒数 | `None` | 让程序等待，避免请求过于频繁 |

网络代码的报错不一定是语法错：断网、网址失效、服务器拒绝、编码判断错误都可能发生。本章先用本地 HTML 学解析，就是为了把“解析是否正确”和“网络是否可用”分开检查。

### 本章分区导航

| 分区 | 对应小节 | 你要抓住的主线 | 产出证据 |
| --- | --- | --- | --- |
| 第一部分：Web 的公共空间和采集边界 | 8.1-8.3 | 爬虫工作于在地址、文档、链接和规则之间 | Web 历史图、核心比喻、边界故事 |
| 第二部分：把爬虫跑起来 | 8.4-8.5 | 先解析本地 HTML，再把规则、来源和结果进行整理 | PowerShell 运行图、最小示例 |
| 第三部分：概念表与脚本导览 | 8.6-8.7 | 每个爬虫概念都要对应到可运行脚本的相应内容 | 概念表、脚本清单、报告输出 |
...
浏览器是人类上网，爬虫是程序替你上网。但程序上网也要守规矩。我们先从画面入手，再慢慢把画面翻译成代码。
...
网页最有意思的地方，就是它既能给人读，也能给程序读。浏览器把 HTML 渲染成漂亮的页面；爬虫则更像是戴着放大镜读原始结构——哪里是标题，哪里是链接，哪里是正文。
...
Tim Berners-Lee 的故事很适合放在爬虫章开头，因为它能把初学爬虫时那种"什么都想抓回来"的兴奋感拉回正轨：Web 不是一座没人看管的仓库，而是一套用地址和链接组织知识的公共空间。爬虫要做的，不是把公共空间搬空，而是把公开、允许、必要的材料整理成可复查的记录。
...
1945 年，Vannevar Bush 写下《As We May Think》，想象了一种叫 Memex 的知识机器：人不是一本接一本地线性翻阅，而是沿着关联线索跳转、记录、再返回。我们在爬虫章节讲这个故事，不是为了考历史，而是想提醒你：链接不只是"随手点的蓝色文字"，它是知识之间的道路。程序沿着道路采集资料时，也要留下路标，不然下次回头就会迷路。
...
如果把网页想象成一座城市，URL 就是门牌号，HTML 是房屋结构，链接是街道。人类用浏览器逛街，Python 用代码按门牌访问。
...
这个画面对应本章的核心比喻：爬虫就像守规矩的资料借阅员——先看地址和规则，再请求页面，取出标题和链接，最后留下可复查的记录。
...
| 顺序 | 主题 | 你要完成的动作 |
| --- | --- | --- |
| 1 | HTTP 请求 | 用 Python 拿着 URL 去敲门，先确认服务器有所回应 |
| 2 | HTML 结构 | 把网页从"漂亮页面"看成一棵标签树 |
| 3 | robots 和边界 | 采集前先读规则，知道哪些地方该停下 |
| 4 | 解析标题与链接 | 从 `<a>` 标签里拆出标题、地址和可复查信息 |
| 5 | 保存 CSV | 把链接清单落到文件里 |
| 6 | 异常处理 | 给网络失败、页面变化和空结果预留处理方案 |
...
先用白话说：爬虫就像守规矩的资料借阅员——先看地址和规则，再请求页面，取出标题和链接，最后留下可复查的记录。
...
看到服务器照片时，可以把一次请求想象成一次非常正式的借阅：你拿着 URL 去找服务器，服务器根据规则把资源交给你。你不应该把图书馆书架整排抱走，也不应该无视门口写着“此处请勿进入”的提示。爬虫的技术能力需要伦理边界。
...
Mosaic 让早期 Web 更容易被普通人使用。对 Python 来说，页面漂不漂亮不重要，重要的是背后的标签结构。你看到网页上的蓝色链接，程序看到的是 `<a href="...">`；你看到标题很大，程序看到的是 `<h1>`、`<h2>`。这一层"从画面回到结构"的转换，就是爬虫学习的关键。
...
本章第一件事不是背参数，而是先跑一个最小例子。打开终端，进入本章目录后运行：
...
如果你能看到输出，说明html解析的入口已经打通了。后面所有复杂功能，都得在这个入口上慢慢增加能力。

---

### 8.5 与心理学和科研资料的连接

这一章把例子贴近心理学、科研记录和学习分享，因为这些任务需要清晰的流程：材料来自哪里，采集边界是什么，数据存到哪里，结果如何展示，别人能不能复查。
...
1. **HTML 数据**（第 5-10 行）：脚本开头直接写了一段简短的 HTML，包含一个 `<h1>` 标题和两个 `<a>` 链接。之所以写死在代码里而不是从网络获取，是为了让初学者先排除"网络不通"这个变量，只聚焦在解析本身。
...
程序会打印出从 HTML 中提取到的两个链接地址。输出虽然简单，但验证了一个核心能力：程序能从结构化的文本中准确提取指定信息。这个能力是所有爬虫工作的起点。
...
1. **导入模块**（第 2-4 行）：
   - `import ssl`：Python 标准库中用于处理 SSL/TLS 加密连接的模块。
   - `import certifi`：提供 Mozilla 维护的根证书包，用于验证 HTTPS 服务器证书（需要先用 `pip install certifi` 安装）。
   - `from urllib.request import Request, urlopen`：Python 标准库中用于发送网络请求的模块。
2. **构造请求**（第 6-12 行）：创建一个 `Request` 对象，传入目标 URL `https://www.python.org/robots.txt`，并通过 `headers` 参数设置 `User-Agent`（告诉服务器"我是一个学习爬虫的 Python 脚本"）和 `Accept-Encoding`（告诉服务器"请用原始格式传输，不要压缩"）。设置请求头是个好习惯，相当于主动打招呼，而不是偷偷摸摸地访问。
3. **创建 SSL 上下文**（第 14 行）：`context = ssl.create_default_context(cafile=certifi.where())`——创建一个 SSL 上下文对象，并指定使用 `certifi` 库提供的证书文件。这一步是为了解决某些环境下 SSL 证书验证失败的问题（如系统根证书未正确配置）。初学者可以不用过多关注这一步，了解即可。
4. **发送请求并读取响应**（第 15-19 行）：`urlopen(request, timeout=10, context=context)` 向服务器发送请求并等待响应，`timeout=10` 表示最多等 10 秒，超时就报错，`context=context` 将之前创建的 SSL 上下文传入。成功后，`response` 对象包含三个重要信息：
...
前两个脚本分别完成了"解析 HTML"和"请求网页"的练习。但这个脚本回答的问题是：**采集到的链接怎么保存**？它的做法是把链接写入 CSV 文件——一种可以用 Excel、WPS 或记事本打开的表格格式。
...
直接 `print(links)` 也能看到结果，但关闭终端后就消失了。写入 CSV 文件意味着：数据可以被后续脚本读取、可以被其他工具打开、可以被存档复查。爬虫不是看到链接就结束，只有把结果落到文件里，资料才算真正进入卡片工厂。

**建议节奏**

第一次运行时不要急着改代码。先原样运行，确认能看到输出；第二次再改一个最小参数，比如增加一条链接；第三次再尝试把输出写入 `output/` 或 `reports/`。
...
3. **生成预览图片**（`make_preview` 函数，第 53-83 行）：用 `PIL`（Pillow 库）创建一个 1500×900 像素的图片。代码中逐行绘制背景、标题、副标题、链接卡片（每个链接带序号、标题和 URL），最后加上检查点提示。这张图片可以让报告"一眼看到全貌"，适合放在学习笔记或项目文档中。这里不需要过多关注，理解这个函数的功能即可。
...
4. **生成预览图片**（`make_preview` 函数，第 89-120 行）：用不同颜色区分可信度（绿色=高可信、橙色=需交叉验证、紫色=待检查），让分类结果一目了然。了解函数功能即可，无需过多关注函数实现。

**预期输出**

- `reports/ch08_source_cards.md`：来源卡片报告，每行一条链接带可信度标签。
- `output/ch08_source_cards_preview.png`：卡片样式的预览图。
...
- **第二部分（请求网页）**：`fetch_page` 函数设置自定义 User-Agent，创建 SSL 上下文处理证书验证问题，用 `try-except` 捕获 HTTP 错误、URL 错误和超时异常。
...
1. 本章最重要的三个概念是什么？请用通俗语言解释，不要只背术语。
