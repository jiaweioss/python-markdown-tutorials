...
这一章不求“抓得多”，只求“抓得对”。初学爬虫很容易兴奋过头，就像第一次走进大型图书馆，恨不得把整排书架都搬回家。但真正稳妥的做法是：先看目录和借阅规则，再挑那些自己有权使用、确实需要的书来拿。
...
浏览器是人类上网，爬虫是程序替你上网。但程序上网也要守规矩。这句话不只是说说而已，而是帮你把本章的知识放进真实场景里。初学者最怕一上来就被术语包围，就像走进一栋所有门牌都用缩写写的大楼。我们先从画面入手，再慢慢把画面翻译成代码。
...
网页最有意思的地方，就是它既能给人读，也能给程序读。浏览器把 HTML 渲染成漂亮的页面；爬虫则更像是戴着放大镜读原始结构——哪里是标题，哪里是链接，哪里是正文。真正合格的爬虫不在于“手快”，而在于知道边界、知道频率、知道哪些页面不该碰。
...
Tim Berners-Lee 的故事放在爬虫章开头很合适，因为它能把初学爬虫时那种"什么都想抓回来"的兴奋感拉回正轨：Web 不是一座没人看管的仓库，而是一套用地址和链接组织知识的公共空间。爬虫要做的，不是把公共空间搬空，而是把公开、允许、必要的材料整理成可复查的记录。
...
如果把网页想象成一座城市，URL 就是门牌号，HTML 是房屋结构，链接是街道。人类用浏览器逛街，Python 用代码按门牌访问。区别在于：程序不会自动懂礼貌，所以得把礼貌写进流程里。
...
这个画面对应本章的核心比喻：爬虫就像守规矩的资料借阅员——先看地址和规则，再请求页面，取出标题和链接，最后留下可复查的记录。如果你能先记住这个比喻，后面的概念就不再是干巴巴的定义了。
...
| 5 | 保存 CSV | 把链接清单落到文件里，别让资料只停在屏幕上 |
...
先用人话说：爬虫就像守规矩的资料借阅员——先看地址和规则，再请求页面，取出标题和链接，最后留下可复查的记录。
...
看到服务器照片时，可以把一次请求想象成一次非常正式的借阅：你拿着 URL 去找服务器，服务器根据规则把资源交给你。你不应该把图书馆书架整排抱走，也不应该无视门口写着“此处请勿进入”的提示。爬虫的技术能力和伦理边界得一起学。
...
如果你能看到输出，说明这一章的入口已经打通了。后面所有复杂功能，都是在这个入口上慢慢增加能力。
...
这张截图特意把 `robots.txt` 放进最小示例里。因为爬虫第一课不该是“怎么快”，而该是“先看规则”。只有当你了解网页结构、请求边界和保存结果，后面的自动化采集才不会变成莽撞点击器。

跑完脚本后，还得确认这些结果真的留下来了。下面这张图就像一张采集任务的出库清单：链接 CSV、采集报告和来源卡片都要能找到。爬虫学习最怕“屏幕上滚过去一堆字，然后什么证据都没留下”，所以本章把证据也做成了一张可检查的清单图片。
...
这张梗图正好提醒本章最重要的习惯：抓取结果必须带着来源一起保存。只保存“答案”很危险，因为答案离开上下文后，很快会变成一句来路不明的传言。保存标题、URL、访问时间、采集边界和使用提醒，才像真正的科研材料。
...
这张表的作用，是把“我好像懂了”变成“我知道它在哪用”。学习编程时，最危险的状态不是完全不会，而是听解释时点头，自己动手时发呆。每学一个概念，都要自己问一句：它在本章项目里负责哪一段工作？
...
2. **构造请求**（第 5-9 行）：创建一个 `Request` 对象，传入目标 URL `https://www.python.org/robots.txt`，并通过 `headers` 参数设置 `User-Agent`（告诉服务器"我是一个学习爬虫的 Python 脚本"）和 `Accept-Encoding`（告诉服务器"请用原始格式传输，不要压缩"）。设置请求头是个好习惯，相当于主动打招呼，而不是偷偷摸摸地访问。
