...
<figure align="center">
  <img src="../assets/ch02/ch02_cover.png" alt="第2章封面" style="zoom:50%; display:block; margin:0 auto;" />
...
这一章会讲很多类型和方法。为了不把它读成一长串清单，先把章节边界看清楚：前面建立“类型判断”的眼光，中间分别学习基础类型和容器类型，最后用项目、练习和文件过渡把这些知识串起来。

| 分区 | 章节范围 | 这一部分要解决的问题 |
| --- | --- | --- |
| 本章导读 | 2.1 | 为什么要先看数据形状，再选择类型 |
...
<figure align="center">
  <img src="../assets/ch02/ch02_roadmap.png" alt="第2章知识路线图" style="zoom:50%; display:block; margin:0 auto;" />
...
这一部分先处理“名字”和“材料”的基本问题：哪些词是 Python 自己保留的，哪些名字可以自己起，变量到底指向什么，以及常见类型大致分成哪几类。
...
| `Ellipsis` | 省略号对象 | 高级切片或占位，初学阶段了解即可 |
| `NotImplemented` | 未实现 | 运算协议相关，初学阶段不用深究 |

初学者最常见的是前三个：`True`、`False`、`None`。
...
<figure align="center">
  <img src="../assets/ch02/ch02_variable_label_metaphor.png" alt="变量标签模型" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-3 变量标签模型</strong>：左边是容易误会的“盒子感”，右边是更接近 Python 的“标签感”：名字可以指向同一个对象，也可以重新贴到新对象上。</figcaption>
</figure>

真正学习数据类型，最好尽早把代码放进真实终端里跑一次。

下面这张图里，第一段运行的是变量标签脚本：`a` 和 `b` 先指向同一个整数对象，`b = 3` 以后，`b` 改贴到新对象上。第二段运行的是学习记录小项目：字符串保存姓名，列表保存分数，字典保存整份记录，布尔值保存是否通过。

这就是本章的隐藏主线：每种类型都不是孤零零的知识点，它们会在一个真实小工具里一起出场。

<figure align="center">
  <img src="../assets/ch02/ch02_powershell_data_type_run.png" alt="PowerShell真实运行数据类型脚本" style="zoom:50%; display:block; margin:0 auto;" />
...
课件里列出了多种数据类型：字符串、布尔、整数、浮点数、列表、元组、字典、集合、日期等。
...
先把这张地图放在脑子里：它不是知识点海报，而是后面所有例子的导航牌。

<figure align="center">
  <img src="../assets/ch02/ch02_data_type_atlas.png" alt="数据类型地图" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-6 数据类型地图</strong>：图里只保留类型节点；具体怎么选，交给正文里的任务和例子来判断。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_information_history_claude_shannon.png" alt="Claude Shannon 肖像" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-7 Claude Shannon</strong>：信息论把“消息”变成可以度量、编码和传输的对象；学习数据类型，也是在学习如何给信息选择合适的表示方式。</figcaption>
</figure>

这时可以想象自己拿着一只“类型选择罗盘”。罗盘不会替你写代码，但它会先把方向指清楚：这份数据是要计算、要展示、要按顺序保存、要按名字查找、要判断真假，还是暂时没有值？

<figure align="center">
  <img src="../assets/ch02/ch02_type_compass_preview.png" alt="类型选择罗盘预览" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-8 类型选择罗盘</strong>：看到新数据时，先问它要完成什么任务，再选择 `str`、`list`、`dict`、`bool`、数值类型或 `None`。</figcaption>
</figure>
...
## 第二部分：基础类型：bool、number、str

这一部分开始进入真正的类型选择。`bool` 适合判断，数值适合计算，字符串适合保存和处理文字。它们看起来简单，但几乎所有程序都会反复用到。
...
<figure align="center">
  <img src="../assets/ch02/ch02_history_george_boole.png" alt="George Boole肖像" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-9 George Boole肖像</strong>：布尔值的名字来自数学家 George Boole；今天的 `True` 和 `False`，背后是把复杂推理压缩成“真/假”判断的思想。</figcaption>
</figure>
...
你不是在写一个小玩具，而是在使用一套已经流进现代计算机血液里的逻辑语言。只是 Python 把它包装得很亲切：分数够了，`passed` 就是 `True`；分数不够，`passed` 就是 `False`。

<figure align="center">
  <img src="../assets/ch02/ch02_bool_logic_switchboard.png" alt="布尔逻辑开关台" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-10 布尔逻辑开关台</strong>：`and` 像两盏灯都要亮，`or` 像至少亮一盏，`not` 像把结果翻面。</figcaption>
</figure>
...
fix_duration = 600
trial_count = 24
total_time = fix_duration * trial_count

print(total_time)
```

如果你做心理学实验，`fix_duration = 600` 可以表示注视点持续 600 毫秒。变量名写清楚，代码就会自己解释自己。
...
<figure align="center">
  <img src="../assets/ch02/ch02_number_rounding_chart.png" alt="数值取整图表" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-11 数值取整图表</strong>：取整不是随便抹零，向上、向下和四舍五入各有自己的规则。</figcaption>
</figure>
...
Python 的 `round()` 还有一个细节：当正好遇到 `.5` 时，它采用“银行家舍入”的规则，不总是简单向上。初学阶段先知道这件事，真正遇到精度问题时再细查。
...
<figure align="center">
  <img src="../assets/ch02/ch02_string_material_workbench.png" alt="字符串材料工作台" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-12 字符串材料工作台</strong>：实验说明、被试编号、刺激词、文件路径和日志备注，看起来都是“文字”，在程序里却都是需要认真保存、查找、替换和切片的数据。</figcaption>
</figure>
...
这里有一个实用提醒：参考 MATLAB 教材时，我们借鉴它“用小操作解释工具”的组织方式，但最终内容要落回 Python。比如字符串替换，就可以用来把旧术语、旧路径、旧实验说明批量更新成新的 Python 版本。
...
<figure align="center">
  <img src="../assets/ch02/ch02_string_slice_ruler.png" alt="字符串切片尺" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-13 字符串切片尺</strong>：数字标的是边界，不是“格子本身”；切片要学会看起点和终点。</figcaption>
</figure>
...
## 第三部分：容器类型：list 与 dict

基础类型通常保存一个值，容器类型负责把多个值组织起来。`list` 强调顺序，`dict` 强调用 key 找到 value。读这一部分时，重点不是“方法名有多少”，而是“数据应该怎样被组织”。
...
<figure align="center">
  <img src="../assets/ch02/ch02_psychology_ebbinghaus_memory.png" alt="Hermann Ebbinghaus肖像" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-14 Hermann Ebbinghaus肖像</strong>：记忆研究里经常会收集一串测试结果；这种“有顺序的一排数据”，正是列表最擅长处理的材料。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_list_workbench.png" alt="列表工作台" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-15 列表工作台</strong>：列表像一排有顺序的抽屉，位置从 0 开始；取值、切片、追加和删除都围绕顺序展开。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_punch_card_sorter_photo.png" alt="IBM 080 打孔卡分拣机照片" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-16 IBM 080 打孔卡分拣机</strong>：在电子表格和数据库普及之前，数据也要按字段分拣、排序和归档；今天的 `list` 与 `dict`，其实是在用代码接管这套整理工作。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_nested_data_matryoshka.png" alt="套娃照片" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-17 套娃与嵌套结构</strong>：嵌套列表就像套娃，一层里面还有一层；它很有用，但层数太多时也会让人找不着出口。</figcaption>
</figure>
...
`my_list[0]` 取第一层抽屉，`my_list[0][1]` 再从这个抽屉里取第 2 个元素。
...
<figure align="center">
  <img src="../assets/ch02/ch02_dictionary_card_catalog_photo.png" alt="图书馆卡片目录抽屉" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-18 图书馆卡片目录抽屉</strong>：字典像一套检索系统，不是靠“第几个”去找，而是靠清楚的 key 直接找到对应信息。</figcaption>
</figure>
...
<figure align="center">
  <img src="../assets/ch02/ch02_dict_mapping_card.png" alt="字典映射卡" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-19 字典映射卡</strong>：左边是 key，右边是 value；字典最重要的能力，就是不用数位置，直接按名字查信息。</figcaption>
</figure>
...
这就像查字典：你用“词”去查“解释”。在程序里，你用 key 去查 value。
...
<figure align="center">
  <img src="../assets/ch02/ch02_mini_project_dashboard.png" alt="本章小项目" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-20 本章小项目</strong>：输入、列表、字典、报告连成一条小流水线，数据类型第一次合在一起做事。</figcaption>
</figure>
...
这张图的重点不是“好看”，而是让你看见一件事：只要类型选得清楚，小项目就会从几行零散代码，变成能保存、能解释、能复查的数据记录。

再把这个思路往真实任务推一步：ch1 里只是演示了一个极简 Stroop 反应，ch2 可以开始把它整理成数据包。心理学实验最怕“当时好像跑过，但后来不知道数据在哪里”。所以我们把每一次 trial 都写成一条 `dict`，再把多条 trial 放进一个 `list`。这样一来，反应时、正确性、刺激词、墨水颜色、备注缺失，都能用类型清楚地表达出来。

<figure align="center">
  <img src="../assets/ch02/ch02_stroop_dataset_pack.png" alt="Stroop 数据类型包" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-22 Stroop 数据类型包</strong>：`list` 保存多条 trial，`dict` 保存一条 trial，`str` 保存刺激标签，`float` 保存反应时，`bool` 保存正确与一致判断，`None` 保存暂无备注。</figcaption>
</figure>
...
这一步很重要：从现在开始，数据类型不只是书上的概念，而是实验记录的骨架。没有合适的数据结构，实验结果就像散落在桌上的便签；结构一清楚，后面才能统计、画图、导出报告。

<figure align="center">
  <img src="../assets/ch02/ch02_data_type_specimen_cabinet.png" alt="数据类型标本柜" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-23 数据类型标本柜</strong>：`11_make_data_type_specimen_cabinet.py` 把 Stroop 记录拆成一格一格的类型标本，让你看到每种类型在同一个任务里的角色。</figcaption>
</figure>

如果说前面的数据包像一份实验材料，那么这张“标本柜”就是本章的陈列架。`str` 负责标签，`int` 负责数量，`float` 负责测量，`bool` 负责判断，`None` 负责表示“暂时没有”，`dict` 保存一条完整记录，`list` 把多条记录排成队。它不要求你背完所有方法名，而是训练一种更重要的能力：看到材料，先判断它应该住在哪个格子里。
...
为了避免把类型学成背诵题，本章再补一份“类型选择卡片”。它把 `bool`、数值、字符串、列表、字典和 `None` 做成一组判断问题：看到新数据时，先问它像什么，再决定把它放到哪里。

<figure align="center">
  <img src="../assets/ch02/ch02_type_decision_cards_preview.png" alt="类型选择卡片预览" style="zoom:50%; display:block; margin:0 auto;" />
  <figcaption><strong>图2-24 类型选择卡片预览</strong>：配套脚本会生成 Markdown 卡片和预览图，把“选类型”变成可复习的小工具。</figcaption>
</figure>
...
如果你希望再多一个“看数据选类型”的练习工具，可以运行类型选择罗盘脚本：
...
这份罗盘比卡片更像现场判断表。比如“姓名”适合 `str`，“一周练习分数”适合 `list`，“学生姓名对应总分”适合 `dict`，“是否完成复习”适合 `bool`，“还没预约复习日期”适合 `None`。它把数据类型从“背概念”往“做设计”推了一步。
...
<figure align="center">
  <img src="../assets/ch02/ch02_error_clue_cards.png" alt="数据类型报错线索卡" style="zoom:50%; display:block; margin:0 auto;" />
...
<figure align="center">
  <img src="../assets/ch02/ch02_practice_workbench.png" alt="数据类型练习工作台" style="zoom:50%; display:block; margin:0 auto;" />
...
```text
reports/ch02_data_type_runtime_evidence.md
...
<figure align="center">
  <img src="../assets/ch02/ch02_type_to_file_bridge.png" alt="数据类型到文件管理的过渡示意图" style="zoom:50%; display:block; margin:0 auto;" />
...
最后把本章文件列清楚，并把数据类型自然接到下一章的“文件、文件夹和路径”。到这里，类型不只是程序运行时的临时材料，也开始准备变成可以保存、复查和交接的文件。

### 2.28 本章配套文件

```text
python_tutorial_ch02/
├── README.md
├── chapters/
│   └── ch02_python_data_types.md
├── assets/
│   └── ch02/
│       ├── ch02_cover.png
│       ├── ch02_roadmap.png
...
├── code/
│   └── ch02/
│       ├── 01_constants_keywords.py
│       ├── 02_variables_labels.py
│       ├── 03_bool_numbers.py
│       ├── 04_string_playground.py
│       ├── 05_list_dict_workshop.py
│       ├── 06_learning_record_project.py
│       ├── 07_make_type_decision_cards.py
│       ├── 08_make_type_compass.py
│       ├── 09_make_data_type_lab_receipt.py
│       ├── 10_make_stroop_dataset_pack.py
│       ├── 11_make_data_type_specimen_cabinet.py
│       ├── 12_make_data_type_runtime_evidence.py
│       └── requirements.txt
├── reports/
│   ├── ch02_data_type_lab_receipt.md
│   ├── ch02_data_type_specimen_cabinet.md
│   ├── ch02_data_type_runtime_evidence.md
│   ├── ch02_stroop_dataset_pack.md
│   ├── ch02_type_decision_cards.md
│   └── ch02_type_compass.md
├── output/
│   ├── ch02_data_type_lab_receipt.png
│   ├── ch02_data_type_specimen_cabinet.json
│   ├── ch02_data_type_specimen_cabinet.png
│   ├── ch02_data_type_runtime_evidence.png
│   ├── ch02_stroop_dataset_pack.csv
│   ├── ch02_stroop_dataset_pack.json
│   ├── ch02_stroop_dataset_pack.png
│   ├── ch02_type_decision_cards_preview.png
│   └── ch02_type_compass_preview.png
├── source_notes/
│   ├── source_manifest_ch02.md
│   └── quality_audit_ch02.md
├── scripts/
│   ├── check_links.py
│   └── generate_ch02_visuals.py
└── manifest.json
```
