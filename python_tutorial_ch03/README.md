...
本包是第 3 章《文件读写与文件夹管理》的 Markdown 教材包。
...
- `source_notes/quality_audit_ch03.md`：第 3 章质量检查记录
...
- 正文扩展为 25 张正式配图，所有图片居中显示，图注统一放在图片下方。
- 本轮补强 Vannevar Bush、档案盒、文件柜和档案库背后的“外置记忆”故事，让文件读写不只是 API 操作，而是保存、定位和追踪资料的日常能力。
- 新增 `10_make_path_safety_receipt.py`，在批量复制、移动、删除前生成“路径安全体检记录”，帮助学生养成先检查路径边界的习惯。
- 新增 `ch03_path_safety_receipt.png`，展示真实运行产物，而不是只讲概念。
- 新增 `11_make_ch02_stroop_file_handoff.py`，读取第2章生成的 Stroop JSON/CSV 数据包，复制到 ch3 安全工作区，按后缀归档并生成文件交接记录。
- 新增 `ch03_ch02_stroop_file_handoff.png`，把跨章节数据交接可视化，让文件读写从“打开文件”升级为“整理资料”。
- 新增 `12_make_archive_evidence_board.py` 和 `ch03_archive_evidence_board.png`，把输入区、整理区、输出区、归档清单、成果记录、路径安全和 ch2 数据交接状态汇总成一张可检查的记录板。
...
- `workspace_ch03/output/ch03_archive_receipt.md`：资料归档记录
- `workspace_ch03/output/ch03_path_safety_receipt.md`：路径安全体检记录
- `workspace_ch03/output/ch03_ch02_stroop_file_handoff.md`：ch2 到 ch3 文件交接记录
- `workspace_ch03/output/ch03_archive_evidence_board.png`：归档记录板
