...
def draw_register(path: Path, summary: dict) -> bool:
    ...
    draw.text((92, 70), "资料入库登记册", font=title, fill=ink)
    draw.text((96, 144), "文件流程从入口开始，到记录可复查为止。", font=subtitle, fill=muted)
    ...
    for name, (x, y) in zone_positions.items():
        ...
        tag(x + 26, y + 28, zone_labels[name], color, 132)
        draw.text((x + 235, y + 35), str(data["files"]), font=title, fill=ink)
        draw.text((x + 30, y + 92), "个文件", font=body, fill=muted)
    ...
    draw.text((190, 660), "记录台账", font=h2, fill=ink)
    ...
    for idx, (name, exists) in enumerate(evidence):
        ...
        draw.text((x + 18, y + 7), "就绪" if exists else "缺失", font=small, fill="#FFFFFF")
        draw.text((x + 132, y + 5), record_labels.get(name, name), font=body, fill=ink)
    ...
    draw.text((560, 1008), "由 code/ch03/13_make_material_intake_register.py 生成", font=mono, fill="#8A5A00")
