...
> 本章定位：这是整套 Python 教程的起步页。先别急着写复杂代码，我们先把课程地图、项目文件和资料整理的方法弄明白。方向感先立住，后面写代码时才不会一边学语法，一边找文件。

### 本章真实任务：启动一个科研卡片工厂

从这一章开始，我们不把 Python 当成一堆散装语法来背。先搭一个能继续生长的**科研卡片小作坊**：`input/` 里放原料，`cards/` 里放学习卡片，`output/` 里放生成结果，`reports/` 里放报告和运行记录，`assets/` 里放图片素材。建好这些位置，后面每一章才有地方接力。
...
在搜索引擎出现以前，图书馆常用一格一格的卡片目录来管理书籍。找书时，人们不是一头扎进书架翻找，而是先按作者、题名或主题查卡片。这个画面很适合本课程：Python 一开始要帮你做的，不是炫技，而是把资料变得可检索、可整理、可复用。之后每一章都会给这间小作坊添一件工具：整理文件、生成卡片、分析数据、处理图片，最后导出 Word 或 PPT 报告。
...
这几年 AI 发展得很快。大模型从经常答非所问，发展到能写代码、改文档、生成表格，也能在不少编程任务中给出很有用的第一稿。它带来的不是一句简单的“还要不要学编程”，而是一个更实际的问题：有了 AI 之后，人应该理解到什么程度，才能判断结果是否可靠？
